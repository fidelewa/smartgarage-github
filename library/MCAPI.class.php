...
class MCAPI {
    ...
    * @param array $type_opts optional - 
            ...
            For AutoResponder campaigns, this array should contain:
                ...
                string event-datemerge optional sort of, this est Obligatoire if the event is "date", "annual", "birthday", or "mergeChanged"
    ...
    */
     ...
     * @return array struct of the statistics for this campaign
                ...
                array timewarp If this campaign was a Timewarp campaign, an array of stats from each timezone for it, with the GMT offset as they key. Each timezone will contain:
                    ...
                    int sent the total number of members Livré to in this timezone
     ...
     * @return array domains email domains and their associated stats
                ...
                int Livré Number of deliveries
     ...
     * @param array  $opts optional various parameters which can be used to configure the shared report
            ...
            string company optional - a Nom de la compagnie to be displayed (use of a theme may hide this) - max 255 bytes
     ...
     * for every user a campaign was Livré to.
     *
     * @section Campaign Report Data
     ...
     * @return array list of merge tags for the list
                ...
                bool req Denotes whether the field est Obligatoire (true) or not (false)
     ...
     * @param array $options optional Various options for this merge var. <em>note:</em> for historical purposes this can also take a "boolean"
                    ...
                    boolean req optional indicates whether the field est Obligatoire - defaults to false
     ...
     * @param array $groups The lists of initial group names to be added - at least 1 est Obligatoire and the names must be unique within a grouping. If the number takes you over the 60 group limit, an error will be thrown.
     ...
     * @param array $merge_vars optional merges for the email (FNAME, LNAME, etc.) (see examples below for handling "blank" arrays). Note that a merge field can only hold up to 255 bytes. Also, there are a few "special" keys:
                        ...
                        string phone If your account has the US Phones option set, this <em>must</em> be in the form of NPA-NXX-LINE (404-555-1212). If not, we assume an International number and will simply set the field with what ever number is passed in.
     ...
     * @return array containing the details for the account tied to this API Key
                ...
                array contact Contact details for the account
                    ...
                    string company Nom de la compagnie
     ...
     * @param array $order an array of information pertaining to the order that has completed. Use the following keys:
                string id the Order Id
                string email_id optional (kind of) the Email Id of the subscriber we should attach this order to (see the "mc_eid" query string variable a campaign passes) - either this or <strong>email</strong> est Obligatoire. If both are provided, email_id takes precedence
                string email optional (kind of) the Email Address we should attach this order to - either this or <strong>email_id</strong> est Obligatoire. If both are provided, email_id takes precedence
                double total The Order Total (ie, the full amount the customer ends up paying)
                string order_date optional the date of the order - if this is not provided, we will default the date to now
                double shipping optional the total paid for Shipping Fees
                double tax optional the total tax paid
